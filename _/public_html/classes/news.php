...
class News extends Helper{
	// Инфо для установки
	public $info_module			= 'Модуль новости';
	// версия
	public $lang 				= 'ru';
	// заголовки
	...
	function get_params(){
		global $ob;
		$s = mysql_query("select * from i_block where translit_name='news'");	
		$r = mysql_fetch_array($s);
		...
		if ($ss && mysql_num_rows($ss)>0)
		{
			while($rr = mysql_fetch_array($ss))
			{
				...
				if ($rr["name_en"] == 'show_count_news' && $rr["value"]!='') $this->show_count_news	= $rr["value"]; 	
			}	
		}
	}
	...
	public function index(){
		...
		{
			$this->get_news($this->url["element"], $this->get);	
		}
		else
		{
		// если есть категории
		if ($this->block===true)
		{
			...
			else
			{
				...
				$this->get_list_news($this->url["block"], $this->get);
			}
		}
		else
		{
			// выводим новости категории
			$this->get_list_news('news', $this->get);
		}
	...
	public function get_list_news($name, $params){
		...
		$api->Pag->setvars($this->lang, 
						...
						mysql_result(mysql_query("SELECT COUNT('id') FROM `i_block_elements` 
												   WHERE `id_section`='$id_block' AND `version`='$this->lang' and news_act=1"), 0), 
						$this->per_page, 
						$params['p']);
						
		// составляем запрос
		$query = mysql_query("select * from `i_block_elements` 
							  WHERE `id_section`='$id_block' AND `version`='$this->lang' and news_act=1 
							  ORDER BY `news_date` DESC LIMIT ".$api->Pag->start_from.", $this->per_page");
		...
		if (mysql_num_rows($query)>0)
		{
			...
			while($r=mysql_fetch_array($query))
			{
				...
				$img=$r["news_img"]!=''?'<a href="'.$link.'"><img src="/upload/images/small/'.$r["news_img"].'" width="150"></a>':'';
				$anounce=nl2br(stripslashes($r["news_anounce"]));
				$date=$api->Strings->date($this->lang,$r["news_date"],'sql','datetext');
				?>
                    <div class="news">
                        <table width="100%">
                            <tr>
                                ...
                                if ($this->show_img==1){
									if ($r["news_img"]!='') {
                                ?>
                                    <td width="170" style="vertical-align:top"><?=$img?></td>
                                <?	
									}
                                }
                                ?>
                                    <td align="left" valign="top">
                                        ...
                                        ?>
                                            <h2 style="font-size:20px; cursor:pointer" onclick="location.href='<?=$link?>'"><?=$name?></h2>
                                        <?
                                        }
                                        ?>
                                        <?
	...
	// вывод страницы
	public function get_news($name, $params){
		global $ob;
		global $incom;
		// проверка новости 
		$s=mysql_query("select * from i_block_elements where version='".$this->lang."' 
						and translit_name='".$name."' and news_name!='' and news_act=1 limit 1");
		if ($s && mysql_num_rows($s)==1)
		{
			$r=mysql_fetch_array($s);
			$this->meta_info["title"]=$r["news_name"];
			$this->header_page($this->lang, $this->meta_info);
			echo stripslashes($r["news_text"]);
	...
	public function get_news_main($lang, $limit, $ids=''){
		global $ob;
		global $api;
		if ($ids == '') $ids = $this->id_section; 
		// составляем запрос
		$query = mysql_query("select * from `i_block_elements` 
							  WHERE `id_section`='$ids' AND `version`='$this->lang' and news_act=1 
		...
		if ($query && mysql_num_rows($query)>0)
		{
			while($r=mysql_fetch_array($query))
			{
				...
				$img=$r["news_img"]!=''?'<a href="'.$link.'"><img src="/upload/images/small/'.$r["news_img"].'" width="50"></a>':'';
				$anounce=mb_substr(stripslashes($r["news_anounce"]),0,150,"UTF-8").'...';
				$date=$api->Strings->date($this->lang,$r["news_date"],'sql','datetext');
				?>
                <table width="100%"><tr>
				    <td width="60"><?=$img?></td>
	...
	function install(){
		...
		$tmp_file = '/tmpl/news.tpl';
		$ext_file = '/ru/news.php';
		
		foreach ($this->fields as $k=>$v)
		{
			$query=mysql_query("ALTER TABLE i_block_elements ADD ".$k." ".$v."");
		}
		$sel=mysql_query("select * from i_block where translit_name='news'");
		if ($sel && mysql_num_rows($sel)==0)
		{
			$s=mysql_query("insert into i_block (id_section,version,act_block,name_block,info_block,sort_block,translit_name) 
							values (0,'all','1','Новости','Модуль новостей','50','news')");	
			if ($s)
			{
