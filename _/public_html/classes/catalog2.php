...
class Catalog2 extends Helper{
	...
	public function get_list_catalog($name, $params){
		...
		$query = mysql_query("select * from `i_block_elements` 
							  ...
							  ORDER BY `catalog_sort` ASC LIMIT ".$api->Pag->start_from.", $this->per_page");
		
		$this->title["title"]=$title;
		$this->header_page($this->lang,$this->meta_info);
		if (mysql_num_rows($query)>0)
		{
			$this->get_list_sub_block($id_block, '').'';
			
				// заголовок ьлока в зависимости от языка	
				if ($this->lang == 'ru')
		{
			$nomera = 'Номера';
		...
		else if ($this->lang == 'zh')
		{
			$nomera = '酒店房间';
					...
					echo '<div class="catalog2header">'.$nomera.'</div><div class="clear"></div>';	
					 
					 
			while($r=mysql_fetch_array($query))
			{
				$name=$r["catalog_name"];
				$link=$part_url[0].'catalog/'.$r["translit_name"];
