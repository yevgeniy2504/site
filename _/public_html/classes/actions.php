...
$check_copy22 = explode('/',$_SERVER['REQUEST_URI']);
$check_copy = explode('?',$check_copy22[0]);



$ggg56756 = "?utm_source=yandex&utm_medium=cpc&utm_content=300_250_t&utm_campaign=Ya_Retarget&type={source_type}&source={source}&block={position_type}&position={position}&utm_term={keyword}";
$ggg56756 = '?';
$ff = explode($ggg56756,$_SERVER['REQUEST_URI']);
$_SERVER['REQUEST_URI'] = $ff[0];
 

if ($check_copy[3] == "?utm_source=yandex&utm_medium=cpc&utm_content=300_250_t&utm_campaign=Ya_Retarget&type={source_type}&source={source}&block={position_type}&position={position}&utm_term={keyword}") {
unset($check_copy[3]);
}

if (($check_copy[1] == 'actions') && ($check_copy[3])) {
header('Location: https://www.hotel-uyut.kz/404.php');
die();
}


if (($check_copy[2] == 'actions') && ($check_copy[4])) {
header('Location: https://www.hotel-uyut.kz/404.php');
die();
}
...
class Actions extends Helper{
	...
	public function index(){
		...
		$aras = explode('/actions/', $_SERVER['REQUEST_URI']);
				
		$block_aras = substr($aras[1],0, -1);
		// die($block_aras);
		$sql = "select * from i_block_elements where translit_name='".$block_aras."' and version='".$this->lang."' and cat_act=1 limit 1";
		// $sql = "select * from i_block_elements where translit_name='".$this->url["block"]."' and version='".$this->lang."' and cat_act=1 limit 1";
		// die($sql);
		// die($this->url['block']);
		$s = mysql_query($sql);
	...
	public function get_list_catalog($name, $params){
		...
		$query = mysql_query("select * from `i_block_elements` 
			WHERE `id_section`='$id_block' AND `version`='$this->lang' and cat_act=1 order by cat_sort asc");
		$meta_res = mysql_fetch_array(mysql_query("select * from `i_block_elements` 
			WHERE `id_section`='$id_block' AND `version`='$this->lang' and cat_act=1 order by id asc limit 1"));
		$this->meta_info['title']= $meta_res['page_name'];
		$this->meta_info['desc']= $meta_res['page_desc'];
		$this->meta_info['h1title']= $meta_res['h1title'];
		$this->meta_info['keyw']= $meta_res['page_keyw'];
		...
		if (mysql_num_rows($query)>0)
		{
			...
			{
				$name=$r["cat_name"];
				$link=$this->lang!="ru"?'/'.$this->lang:''; $link.='/actions/'.$r["translit_name"].'/';
	...
	public function get_catalog($name, $params){
		...
		if ($s && mysql_num_rows($s)==1)
		{
			...
			if ($this->lang=='ru'){
				...
				$this->meta_info["title"]=$r["page_name"];
				$this->meta_info["desc"] = 'Алматы қаласының Уют қонақжайындағы  '.$r["cat_name"].' - сипаттамалар, фотолар, пікірлер. Қызмет көрсетудің жоғары деңгейі бар заманауи қонақ үй кешеніне жүгініңіз. Уют Отелі сізді күтуде. Звоните: (727) 279-51-11 ';
				$this->meta_info["keyw"] = str_replace('  ', ' ', lower(str_replace(array('«','»','"', ',', '.', '!', ' и ', ' а ',  ' за ', ' но ', ' и ', ' а ', ' но ', ' да ', ' если ', ' что ', ' когда ', ' в ', ' с ', ' к ', ' у ', ' над ', ' на ', ' перед ', ' при ', ' для '), ' ', $r["cat_name"])).'');
			}
			else{
				$this->meta_info["title"]=$r["page_name"];
