...
class Action extends Helper{
	// Инфо для установки
	public $info_module			= 'Календарь событий';
	// версия
	public $lang 				= 'ru';
	// заголовки
	...
	function get_params(){
		global $ob;
		$s = mysql_query("select * from i_block where translit_name='action'");	
		$r = mysql_fetch_array($s);
		...
		if ($ss && mysql_num_rows($ss)>0)
		{
			while($rr = mysql_fetch_array($ss))
			{
				...
				if ($rr["name_en"] == 'show_count_action' && $rr["value"]!='') $this->show_count_action	= $rr["value"]; 	
			}	
		}
	}
	...
	public function index(){
		...
		{
			$this->get_action($this->url["element"], $this->get);	
		}
		else
		{
		// если есть категории
		if ($this->block===true)
		{
			...
			else
			{
				...
				$this->get_list_action($this->url["block"], $this->get);
			}
		}
		else
		{
			// выводим новости категории
			$this->get_list_action('action', $this->get);
		}
	...
	public function get_list_action($name, $params){
		...
		$api->Pag->setvars($this->lang, 
						...
						mysql_result(mysql_query("SELECT COUNT('id') FROM `i_block_elements` 
												   WHERE `id_section`='$id_block' AND `version`='$this->lang' and action_act=1"), 0), 
						$this->per_page, 
						$params['p']);
						
		// составляем запрос
		$query = mysql_query("select * from `i_block_elements` 
							  WHERE `id_section`='$id_block' AND `version`='$this->lang' and action_act=1 
							  ORDER BY `action_date` DESC LIMIT ".$api->Pag->start_from.", $this->per_page");
		...
		if (mysql_num_rows($query)>0)
		{
			...
			while($r=mysql_fetch_array($query))
			{
				...
				$img=$r["action_img"]!=''?'<a href="'.$link.'"><img src="/upload/images/small/'.$r["action_img"].'" width="150"></a>':'';
				$anounce=nl2br(stripslashes($r["action_anounce"]));
				$date=$api->Strings->date($this->lang,$r["action_date"],'sql','datetext');
				?>
                    <div class="action">
                        <table width="100%">
                            <tr>
	...
	// вывод месяцев для навигации
	public function get_month(){
		$j=0;
		$insertsql="SELECT DATE_FORMAT(action_date,'%Y') as 'year',DATE_FORMAT(action_date,'%m') as 'month' 
				    FROM i_block_elements where action_act=1 and version ='".$this->lang."' 
					GROUP BY DATE_FORMAT(action_date,'%Y'),DATE_FORMAT(action_date,'%m') 
					ORDER BY DATE_FORMAT(action_date,'%Y'),DATE_FORMAT(action_date,'%m') ASC";
        $month_name[1] = 'Январь';       
        ...
        $last_y=$months='';$empt_month=1;
        $Res=mysql_query($insertsql);       
        echo '<table width="100%"><tr><td class="arhiv"><p>Архив новостей</p></td></tr>';
			while($row=mysql_fetch_array($Res)){           
            	if($last_y!=$row["year"]) {
                	echo '<tr><td><p><b>'.$row["year"].'</b></p></td></tr>';     
                    $months='';    
                    $empt_month=1; 
                }
				while ($row["month"]>$empt_month) 
				{ 
					$empt_month++;
				}
                $r_m=trim($row["month"],'0');
                $months.=' '."<a href='?year=".$row["year"]."&month=".$row["month"]."'>".$month_name[$r_m].'</a>';
                $empt_month++;
                $last_y=$row["year"];
                $last_month=$row["month"];
       		}      
       		if(!empty($months)) print("<tr><td><p>".trim($months,' - ')."</p></td></tr>");
...
<?
	...
	// вывод страницы
	public function get_action($name, $params){
		global $ob;
		global $incom;
		// проверка новости 
		$s=mysql_query("select * from i_block_elements where version='".$this->lang."' 
						and translit_name='".$name."' and action_name!='' and action_act=1 limit 1");
		if ($s && mysql_num_rows($s)==1)
		{
			$r=mysql_fetch_array($s);
			$this->meta_info["title"]=$r["action_name"];
			$this->header_page($this->lang, $this->meta_info);
			echo stripslashes($r["action_text"]);
	...
	public function get_action_main($lang, $limit, $ids=''){
		global $ob;
		global $api;
		if ($ids == '') $ids = $this->id_section; 
		// составляем запрос
		$query = mysql_query("select * from `i_block_elements` 
							  WHERE `id_section`='$ids' AND `version`='$this->lang' and action_act=1 
							  ORDER BY `action_date` DESC LIMIT ".$limit."");
		if ($query && mysql_num_rows($query)>0)
		{
			while($r=mysql_fetch_array($query))
			{
				...
				$img=$r["action_img"]!=''?'<a href="'.$link.'"><img src="/upload/images/small/'.$r["action_img"].'" width="50"></a>':'';
				$anounce=mb_substr(stripslashes($r["action_anounce"]),0,150,"UTF-8").'...';
				$date=$api->Strings->date($this->lang,$r["action_date"],'sql','datetext');
				?>
                <table width="100%"><tr>
				    <td width="60"><?=$img?></td>
	...
	function install(){
		...
		$tmp_file = '/tmpl/action.tpl';
		$ext_file = '/ru/action.php';
		
		foreach ($this->fields as $k=>$v)
		{
			$query=mysql_query("ALTER TABLE i_block_elements ADD ".$k." ".$v."");
		}
		$sel=mysql_query("select * from i_block where translit_name='action'");
		if ($sel && mysql_num_rows($sel)==0)
		{
			$s=mysql_query("insert into i_block (id_section,version,act_block,name_block,info_block,sort_block,translit_name) 
							values (0,'all','1','Календарь событий','Модуль календарь событий','50','action')");	
			if ($s)
			{
