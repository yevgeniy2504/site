...
class Gallery extends Helper{
	// версия
	public $info_module = 'Фотогалерея';
	// версия
	public $lang = 'ru';
	// заголовки
	...
	public $fields 		  =	array('gal_act'		=>'tinyint(4)',
						   		  'gal_name'	=>"varchar(255)",
								  'gal_img'		=>"text");
	//поля для модуля для optiona
	public $option_fields = array('gal_act'	    =>array("type_field"=>'i7',
												 "select_fields"=>"1",
												 "name_ru"=>"Активность",
												 "id_sort"=>10),
								  'gal_name'	=>array("type_field"=>'i1',
												 "select_fields"=>"1",
												 "required_fields"=>"1",
												 "name_ru"=>"Название",
												 "width"=>"30",
	...
	function get_params(){
		global $ob;
		$s = mysql_query("select * from i_block where translit_name='gallery'");	
		$r = mysql_fetch_array($s);
	...
	public function index(){
		...
		{
			$this->get_gallery($this->url["element"], $this->get);	
		}
		else
		{
			// если есть категории
			if ($this->block===true)
			{
				...
				else
				{
					...
					$this->get_list_gallery($this->url["block"], $this->get);
				}
			}
			else
			{
				// выводим галереи категории
				$this->get_list_gallery('gallery', $this->get);
			}
		}
	}
	
	// вывод новостей постранично
	public function get_list_gallery($name, $params){
		global $api;
		// получаем парамметры 
		...
		$api->Pag->setvars($this->lang, 
						...
						mysql_result(mysql_query("SELECT COUNT('id') FROM `i_block_elements` 
												   WHERE `id_section`='$id_block' AND `version`='$this->lang' and gal_act=1"), 0), 
						$this->per_page, 
						$params['p']);
		// составляем запрос
		$query = mysql_query("select * from `i_block_elements` 
							  WHERE `id_section`='$id_block' AND `version`='$this->lang' and gal_act=1 
		...
		if (mysql_num_rows($query)>0)
		{
			...
			while($r=mysql_fetch_array($query))
			{
				...
				$img=$r["gal_img"]!=''?'<a '.($this->show_comment==1?'href="'.$link.'"':'rel="gallery" href="/upload/images/'.$r["gal_img"].'"').' title="'.$name.'"><img src="/upload/images/small/'.$r["gal_img"].'" ></a><br />':'';
				?>
	...
	public function get_list_sub_block($ids, $params){
		...
		if ($s && mysql_num_rows($s)>0)
		{
			...
			while($r=mysql_fetch_array($s))
			{
				...
				$img=$r["img_block"]!=''?'<a href="вв'.$link.'" title="'.$name.'">
						<img src="/upload/images/small/'.$r["img_block"].'" ></a><br />':'';
				?>
			<td width="33%" align="center" valign="middle" style="padding:10px;"><?=$img?>
            <?=$name?><br />
...
?></td>
	...
	// вывод страницы
	public function get_gallery($name, $params){
		...
		global $incom;
				...
				$s=mysql_query("select * from i_block_elements where version='".$this->lang."' 
								and translit_name='".$name."' and gal_name!='' and gal_act=1 limit 1");
				if (mysql_num_rows($s)==1)
				{
					$r=mysql_fetch_array($s);
					
					$this->meta_info["title"]=$r["gal_name"];
					
					$this->header_page($this->lang, $this->meta_info);
					$img=$r["gal_img"]!=''?'<a rel="gallery" href="/upload/images/'.$r["gal_img"].'" title="'.$name.'">
						<img src="/upload/images/big/'.$r["gal_img"].'" ></a><br />':'';
	...
	// установка модуля	
	function install(){
		$tmp_file = '/tmpl/gallery.tpl';
		$ext_file = '/ru/gallery.php';
		...
		$s=mysql_query("insert into i_block (id_section,version,act_block,name_block,info_block,sort_block,translit_name) 
									values (0,'all','1','Фотогалерея','Фотогалерея','20','gallery')");	
