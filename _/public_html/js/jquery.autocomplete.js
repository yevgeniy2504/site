...
jQuery.fn.autocomplete = function(url, settings ) 
{
	return this.each( function()//do it for each matched element
	{
		...
		textInput.after('<input type="hidden" id="' + textInput.attr("name") + '" name="' + textInput.attr("name") + '_name"/>');
		var valueInput = $(this).next();
		//create the ul that will hold the text and values
		var list = $('<ul>').addClass('autocomplete').insertAfter(valueInput)
			.css({
				top: textInput.position().top + textInput.outerHeight(), 
			...
			if (text != oldText && (settings.minChars != null && text.length >= settings.minChars))
			{
				...
				$.getJSON(url,settings.parameters,function(data)
				{
					...
					if (data)
					{
						...
						{
							var row = data[i];
						  items = '<li value="' + row.id + '">' + row.text.replace(new RegExp("(" + text + ")","i"),"<strong>$1</strong>") + '</li>';
						  list.append(items);
