...
if(@$_POST['actions'])
{
	if(count(@$_POST['select_ch'])>0)
	{
		...
		{
		if($_POST['actions']=="action"){$query="update i_template set active='1' where id='".$ob->pr($v)."'";}
		if($_POST['actions']=="deaction"){$query="update i_template set active='0' where id='".$ob->pr($v)."'";}
		if($_POST['actions']=="delete"){$ob->del_r("i_template",array("id"=>$v));}
		mysql_query(@$query);
		}
...
{
	$ob->update("i_template",array("active"=>$_GET['active']),$_GET['id'],"");
}

if(@$_GET['delete']=="true" AND $_GET['id'])
{
$ob->del_r("i_template",array("id"=>$_GET['id']));
}

//началась выборка
$field=array("active"=>"Активность","name"=>"Наименование","version"=>"Версия");
$where='';
...
$select=mysql_query("select * from i_template where id<>0 ".$where." ORDER BY ".$ord.",'puttime' DESC LIMIT ".$start.",".$number."");
$select_all=mysql_query("select * from  i_template where id<>0 ".$where."");
?>
      ...
      <td height="29" align="center" class="panel_back" style="border: 1px solid #c4c5a6;"><table width="98%" border="0" cellspacing="0" cellpadding="0">
          ...
          <tr>
            ...
            <td width="21%" align="center" onmouseover="this.className='cell_border_small';" onmouseout="this.className='small_text';"><table width="90%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td width="16%" align="center"><a href="add.php"><img src="/incom/modules/theme/default/images/blank.gif" width="16" height="21" border="0" /></a></td>
                  <td width="84%" align="center" class="small_text"><a href="add.php" class="small_text">Добавить шаблон</a></td>
                </tr>
            </table></td>
            <td width="40%" align="center" >&nbsp;</td>
            <td width="37%" align="center" ><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
...
';
	...
	background-position: left top; text-align:left;	padding-left: 23px;"';
      ...
      <ul>
		<li '.$li_style.'><a href="view.php'.$ob->gets_go($_GET,"id",$res['id']).'" target="_blank">просмотреть</a></li>
...
//-->
        ...
        <td class="table_value">'.$res['version'].'&nbsp;</td>
      </tr>';
	  }
	  ?>
      </table></td>
