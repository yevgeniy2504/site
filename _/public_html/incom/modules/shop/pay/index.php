...
if(@$_POST['actions'])
{
	if(count(@$_POST['select_ch'])>0)
	{
		...
		{
		if($_POST['actions']=="true"){$query="update i_payments set paid='1' where id='".$ob->pr($v)."'";}
		if($_POST['actions']=="false"){$query="update i_payments set paid='0' where id='".$ob->pr($v)."'";}
		if($_POST['actions']=="delete"){$ob->del_r("i_payments",array("id"=>$v));}
		mysql_query(@$query);
		}
...
//началась выборка
$field=array("timestamp_x"=>"Дата/Время","paid"=>"Статус","goods"=>"Кол-во товаров","id_user"=>"Логин");
$where='';
...
$select=mysql_query("select * from i_payments where id<>0 ".$where." ORDER BY ".$ord.",'puttime' DESC LIMIT ".$start.",".$number."");
$select_all=mysql_query("select * from  i_payments where id<>0 ".$where."");
?>
...
<form id="form_index" name="form_index" method="post" action="index.php" >
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
      <td height="35" class="title_text">Текущие платежи</td>
    </tr>
    <tr>
    ...
    <tr>
      <td height="29" align="center" class="panel_back" style="border: 1px solid #c4c5a6;"><table width="98%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td width="47%">&nbsp;</td>
          <td width="35%" align="center"><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td align="center"><select name="actions" id="actions" style="width:130px">
                    <?
            $actions=array(""=>"выбор действия","true"=>"оплаченный","false"=>"неоплаченный","delete"=>"удалить");
...
//-->
		...
		<td class="table_value">'.$res['timestamp_x'].'&nbsp;</td>
        <td class="table_value">'.$act.'&nbsp;</td>
        <td class="table_value">'.$count_goods.'&nbsp;</td>
        <td class="table_value">'.$user_res['login'].'&nbsp;</td>
      </tr>';
	  }
	  ?>
      </table></td>
