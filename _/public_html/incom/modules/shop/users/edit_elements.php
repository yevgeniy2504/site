...
$field['password']=$_POST['password'];
	...
	$select=$ob->select("i_shop_users",array("login"=>$_POST['login']),"");
		...
		$field['password']=$_POST['password'];
		//$ob->alert($field['password']);
		}else
		{
		$erorr="Извините, но данный логин уже существует!";
		}
	}
	
	
	
	$option=$ob->select("i_option",array("category"=>"shop_users","category_id"=>$_GET['id_section']),"id_sort");
	...
	while($option_res=mysql_fetch_array($option))
	{
		...
		if($option_res['type_field']=="i10" OR $option_res['type_field']=="i11")
		{
				...
				{
				$delete=$ob->select("i_shop_users",array("id"=>$_GET['id']),"");
	...
	$field_stat=array("active"=>$_POST['active'],"mail"=>$_POST['mail']);
		
	if(!@$erorr)
	{	
	$ob->update("i_shop_users",array_merge($field,$field_stat),$_GET['id'],$page);
	}else
	{
	$ob->alert($erorr);
	}
...
<?
$select=$ob->select("i_shop_users",array("id"=>$_GET['id']),"");
          ...
          <tr>
            <td align="center"><table width="100%" border="0" cellspacing="0" cellpadding="4">
                ...
                <tr>
                  ...
                  <td align="left"><input name="mail" type="text" id="mail" value="<?=$res['mail']?>" size="35" /></td>
                </tr>
                <?
