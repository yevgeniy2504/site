...
if(@$_POST['actions'])
{
	if(count(@$_POST['select_ch'])>0)
	{
		foreach($_POST['select_ch'] as $k=>$v)
		{
			...
			{
			$ob->del_r("i_shop_group",array("id"=>$v));
			$ob->del_r("i_shop_users",array("id_section"=>$v));
			}
		}
...
{
$ob->del_r("i_shop_group",array("id"=>$_GET['id']));
$ob->del_r("i_shop_users",array("id_section"=>$_GET['id']));
}

//началась выборка
$field=array();
$field['#']="Пользователей";
$select=$ob->search_option("shop_group","",@$_GET['id_section'],array("select_fields"=>1));
$m=0;
...
$select=mysql_query("select * from i_shop_group where (version='all' OR version='".$_SESSION['version']."') ".$where." ORDER BY ".$ord.",'puttime' DESC LIMIT ".$start.",".$number."");
$select_all=mysql_query("select * from  i_shop_group where (version='all' OR version='".$_SESSION['version']."') ".$where."");
?>
...
<form id="form_index" name="form_index" method="post" action="">
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
      <td height="35" class="title_text">Группы пользователей</td>
    </tr>
    <tr>
                            ...
                            <tr >
                              <td align="left"><table width="100%" border="0" cellspacing="0" cellpadding="0">
								 ...
								 foreach($field as $k=>$v)
								 {
								  if($i!=0)
								 {
								 if(@$_POST['field_'.$k.'']==1){$view="shop_group";}else{$view="none";}
...
?></td>
      ...
      <td height="29" align="center" class="panel_back" style="border: 1px solid #c4c5a6;"><table width="98%" border="0" cellspacing="0" cellpadding="0">
          ...
          <tr>
            ...
            <td width="20%" align="center" onmouseover="this.className='cell_border_small';" onmouseout="this.className='small_text';"><table width="90%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td width="16%" align="center"><a href="add.php<?=$ob->gets_go($_GET,"module","shop_group")?>"><img src="/incom/modules/theme/default/images/blank.gif" alt="добавить информ. блок" width="16" height="21" border="0" /></a></td>
                  <td width="84%" align="center" class="small_text"><a href="add.php<?=$ob->gets_go($_GET,"module","shop_group")?>" class="small_text">Добавить группу</a></td>
                </tr>
            </table></td>
            ...
            <td width="16%" align="center" onmouseover="this.className='cell_border_small';" onmouseout="this.className='small_text';"><table width="90%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td width="37%" align="center"><a href="/incom/modules/inf_block/option.php?module=shop_group&amp;id_section=<?=@$_GET['id_section']?>"><img src="/incom/modules/theme/default/images/option.gif" alt="настройка инфор. блока" width="23" height="22" border="0" /></a></td>
                <td width="63%" align="center" class="small_text"><a href="/incom/modules/inf_block/option.php?module=shop_group&id_section=<?=@$_GET['id_section']?>" class="small_text">Настроить</a></td>
              </tr>
            </table></td>
   ...
   echo '<tr>
	...
	<tr>
	  ...
	  $hide_ar=array();
	  $hide_ar2=array();
	  $field_name=$ob->select("i_option",array("category"=>"shop_group","select_fields"=>1),"id_sort");
	  ...
	  for($i=0;$i<=4;$i++)
	  {
	  	if($i==0)
	  	{
	  	$hide=$ob->select("i_shop_group",array("id"=>$_GET['id_section']),"");
	  	$hide_res=mysql_fetch_array($hide);
	  	array_push($hide_ar,$hide_res[''.$hide_field_name.'']);
		array_push($hide_ar2,'index.php');
		}else
		{
		$hide=$ob->select("i_shop_group",array("id"=>$hide_res['id_section']),"");
	  	$hide_res=mysql_fetch_array($hide);
		array_push($hide_ar,$hide_res[''.$hide_field_name.'']);
	  ...
	  $m++;
	  	...
	  	{
		$i++;
		if($m!=count($hide_ar)){echo '<a href="'.$hide_ar2[''.$i.''].'&module=shop_group" class="small_red_text">'.$v.'</a> / ';}else{echo ''.$v.' / ';}
...
';
      ...
      while($res=mysql_fetch_array($select))
	  {
	  $elements=$ob->select("i_shop_users",array("id_section"=>$res['id'],"version"=>$_SESSION['version']),"");
	...
	background-position: left top; text-align:left;	padding-left: 23px;"';
	  ...
	  echo '<tr onMouseOver="this.bgColor=\'#f3f2da\'" onMouseOut="this.bgColor=\'#fafaf0\'" onmouseup="if (!stopMoving()) if (!otherClicks(event)) return CheckTR(this);" onmousedown="startMoving()" id="str'.$res['id'].'" ondblclick="javascript:document.location.href=\'edit.php'.$ob->gets_go($_GET,"id",$res['id']).'&module=shop_group\'" >
      ...
      <ul>
		...
		<li '.$li_style.'><a href="elements.php?module=shop_users&sub_module=shop_group&id_section='.$res['id'].'">пользователи</a></li>
		<li '.$li_style.'><a href="/incom/modules/inf_block/option.php?module=shop_users&id_section='.$res['id'].'">настройка</a></li>
		<li '.$li_style.'><a href="edit.php'.$ob->gets_go($_GET,"id",$res['id']).'&module=shop_group">редактировать</a></li>
...
<td class="table_value"> <a href="elements.php?module=shop_users&sub_module=shop_group&id_section='.$res['id'].'" style="color:#72ad0f;font-weight:bold;">'.mysql_num_rows($elements).'</a></td>';
		$field2=$ob->search_option("shop_group","",@$_GET['id_section'],array("select_fields"=>1));
...
if(@$_GET['excel']=="true")
{
$select=mysql_query("select * from i_shop_group where (version='all' OR version='".$_SESSION['version']."') ".$where." ORDER BY ".$ord.",'puttime' DESC LIMIT ".$start.",".$number."");$str='<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
