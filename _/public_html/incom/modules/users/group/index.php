...
//началась выборка
$field=array("active"=>"Активность","name"=>"Наименование","info"=>"Описание");
$where='';
...
if(!@$_GET['order']){$ord="id_sort";}else{$ord=$ob->pr($_GET['order']);}
if(!@$_GET['start']){$start=0;}else{$start=$ob->pr($_GET['start']);} 
if(!@$_GET['number']){$number=10;}else{$number=$ob->pr($_GET['number']);}

$select=mysql_query("select * from i_user_group where id<>0 ".$where." ORDER BY ".$ord.",'puttime' DESC LIMIT ".$start.",".$number."");
$select_all=mysql_query("select * from  i_user_group where id<>0 ".$where."");
?>
      ...
      <td height="29" align="center" class="panel_back" style="border: 1px solid #c4c5a6;"><table width="98%" border="0" cellspacing="0" cellpadding="0">
          ...
          <tr>
            ...
            <td width="21%" align="center" onmouseover="this.className='cell_border_small';" onmouseout="this.className='small_text';"><table width="90%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td width="16%" align="center"><a href="add.php"><img src="/incom/modules/theme/default/images/blank.gif" alt="добавить группу" width="16" height="21" border="0" /></a></td>
                  <td width="84%" align="center" class="small_text"><a href="add.php" class="small_text">Добавить группу</a></td>
                </tr>
            </table></td>
            ...
            <td width="27%">&nbsp;</td>
            <td width="36%" align="center"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
...
//-->
        ...
        <td class="table_value">'.$res['info'].'&nbsp;</td>
         </tr>';
	  }
	  ?>
      </table></td>
