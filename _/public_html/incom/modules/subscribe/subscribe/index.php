...
if(@$_POST['actions'])
{
	if(count(@$_POST['select_ch'])>0)
	{
		...
		{
		if($_POST['actions']=="action"){$query="update i_subscribes set act='1' where id='".$ob->pr($v)."'";}
		if($_POST['actions']=="deaction"){$query="update i_subscribes set act='0' where id='".$ob->pr($v)."'";}
		if($_POST['actions']=="delete"){$ob->del_r("i_subscribes",array("id"=>$v));}
		mysql_query(@$query);
		}
	}else{$ob->alert("Необходимо выбрать элементы таблицы!");}
}

if(@$_GET['id'] AND isset($_GET['act']))
{
	$ob->update("i_subscribes",array("act"=>$_GET['act']),$_GET['id'],"");
}

if(@$_GET['delete']=="true" AND $_GET['id'])
{
$ob->del_r("i_subscribes",array("id"=>$_GET['id']));
}

//началась выборка
$field=array("act"=>"Активность","date"=>"Дата", "name"=>"Название","theme"=>"Тема");
$where='';
...
$select=mysql_query("select * from i_subscribes where id<>0 ".$where." AND `version`='".$_SESSION['version']."' ORDER BY ".$ord.",'puttime' DESC LIMIT ".$start.",".$number."");
$select_all=mysql_query("select * from  i_subscribes where id<>0 AND `version`='".$_SESSION['version']."' ".$where."");
?>
...
<form id="form_index" name="form_index" method="post" action="index.php" >
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
      <td height="35" class="title_text">Список рассылок</td>
    </tr>
    <tr>
          ...
          <tr>
            ...
            <td width="26%" align="center" onmouseover="this.className='cell_border_small';" onmouseout="this.className='small_text';"><table width="90%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  ...
                  <td width="84%" align="center" class="small_text"><a href="add.php" class="small_text">Добавить рассылку</a></td>
                </tr>
            </table></td>
            ...
            <td width="15%" align="center" onmouseover="this.className='cell_border_small';" onmouseout="this.className='small_text';"><table width="90%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td width="37%" align="center"><a href="option.php"><img src="/incom/modules/theme/default/images/option.gif" alt="настройка пользователей" width="23" height="22" border="0" /></a></td>
                <td width="63%" align="center" class="small_text"><a href="option.php" class="small_text">Настроить</a></td>
              </tr>
            </table></td>-->
            <td width="32%">&nbsp;</td>
...
';
	...
	background-position: left top; text-align:left;	padding-left: 23px;"';
	  ...
	  if($res['act']==1){$act='Да';}else{$act='Нет';}
      ...
      <ul>
		<li '.$li_style.'><a href="'.$ob->gets_go($_GET,"id",$res['id']).'&act=1">активизировать</a></li>
		<li '.$li_style.'><a href="'.$ob->gets_go($_GET,"id",$res['id']).'&act=0">деактивизировать</a></li>
...
//-->
		...
		<td class="table_value">'.$res['date'].'&nbsp;</td>
        <td class="table_value">'.$res['name'].'&nbsp;</td>
        <td class="table_value">'.$res['theme'].'&nbsp;</td>
      </tr>';
	  }
	  ?>
      </table></td>
