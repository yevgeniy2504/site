...
{
	if ($_POST["module"]!='' && $_POST["templates"]!='' && $_POST["ids"]!='' && $_POST["ids"]!=0)
	{
		//vd($_POST);
		
		$s=mysql_query("select * from i_option where category='templates' and category_id=".intval($_POST["templates"])."");	
		if ($s)
		{
			while($r=mysql_fetch_array($s))
			{
				...
				$test=mysql_query("select * from i_option where category='".$_POST["module"]."' 
									and category_id='".$_POST["ids"]."' and name_en='".$r["name_en"]."'");
				if (mysql_num_rows($test)==0)
				$i = mysql_query("insert into i_option values ('','".$_POST["module"]."','".$_POST["ids"]."',
				...
				if($r['type_field']=="i1" OR $r['type_field']=="i3" OR $r['type_field']=="i4" 
					OR $r['type_field']=="i7"  OR $r['type_field']=="i11" OR $r['type_field']=="i12"){
				...
				$res = mysql_query("SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='".$db."' 
									AND TABLE_NAME='i_".$_POST["module"]."' AND COLUMN_NAME='".$r["name_en"]."'");
				...
				{
					$i=mysql_query("ALTER TABLE i_".$_POST['module']." ADD ".$ob->pr($r['name_en'])." ".$type_res."");
		...
		if($_POST['type_field']!="i0"){
			...
			// если редактирование
				...
				if($i){
					...
					$ob->alert("Ошибка создания поля!".mysql_error());
...
<form id="form" name="form" method="post" action="<?=$ob->gets_go($_GET,"","")?>" enctype="multipart/form-data" >
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr>
      <td align="left" bgcolor="#f9f8e8" style="border: 1px solid #c4c5a6;"><table  align="left" width="300" border="0" cellspacing="4" cellpadding="0">
          <tr>
      ...
      <form id="templates" name="templates" method="post" action="<?=$ob->gets_go($_GET,"","")?>" enctype="multipart/form-data" class="small_text" style="float:right" >
      <input type="hidden" value="<?=@$_GET["module"]?>" name="module">
      <input type="hidden" value="<?=@$_GET["id_section"]?>" name="ids">
      Применить шаблон <select name="templates" id="templates" style="width:200px;">
      <option value="">Выберите шаблон</option>
      <?
      $s=mysql_query("select * from i_templates order by sort_templates asc");
	  if ($s)
	  {
	      while($r=mysql_fetch_array($s))
		  {
      ...
      &nbsp;
      &nbsp;
      <input type="submit" name="app_templates" id="app_templates" value="Применить шаблон">
      </form>
      </td>
    </tr>
    <tr>
