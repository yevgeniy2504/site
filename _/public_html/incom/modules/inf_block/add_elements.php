...
$i=1;
			...
			if($option_res['type_field']=="i12"){$folder="files";}
				...
				if(!empty($_FILES[''.$option_res['name_en'].'']['tmp_name']))
				{
					...
					if(in_array(getExtension($_FILES[''.$option_res['name_en'].'']['name']),$format))
						{
						$text=rand(0,100000)."_".rand(500,1000000)."_".date("H");
						$upload=move_uploaded_file($_FILES[''.$option_res['name_en'].'']['tmp_name'],$_SERVER['DOCUMENT_ROOT']."/upload/".$folder."/".$text.'.'.getExtension($_FILES[''.$option_res['name_en'].'']['name']));
						$field.="'".$text.'.'.getExtension($_FILES[''.$option_res['name_en'].'']['name'])."'";
	...
	break;
			...
			if (@$option_res["translit"]==1) $translit=translit(@$_POST[''.$option_res['name_en'].''].'_'.$_GET["id"]);
	...
	$i=mysql_query("INSERT INTO i_".$ob->pr($_GET['module'])." (id_section,version,translit_name,".$field_name.") VALUES ('".@$_GET['id_section']."','".$_SESSION['version']."','".$translit."'".@$field.")");
				...
				if($option_res['required_fields']==1){$star='<span class="small_red_text">*</span>';}else{$star='';}
				  ...
				  if ($s)
				  {
					while($r=mysql_fetch_array($s))
					{
						...
						echo'<option value="'.$r["id"].'">'.$r["brand_name"].'</option>';
				  ...
				  echo'</select></td>
					...
					if ($option_res['name_en']=='link_menu')
					{
						...
						echo '<tr>
								...
								$s=mysql_query("(select page_name, translit_name from i_block where page_act=1 and (version='".$_SESSION["version"]."' or version='all')) UNION ALL (select page_name, translit_name from i_block_elements where page_act=1 and version='".$_SESSION["version"]."') order by page_name asc");
								if ($s)
								{
									while($r=mysql_fetch_array($s))
									{
